...
/// Subscription topic delimiter for Architect.
const ARCHITECT_TOPIC_DELIMITER: char = ':';

/// Result type for Architect WebSocket operations.
pub type ArchitectWsResult<T> = Result<T, ArchitectWsClientError>;

/// Error type for the Architect WebSocket client.
#[derive(Debug, Clone)]
pub enum ArchitectWsClientError {
...
impl core::fmt::Display for ArchitectWsClientError {
...
impl std::error::Error for ArchitectWsClientError {}
...
impl Debug for ArchitectMdWebSocketClient {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        f.debug_struct("ArchitectMdWebSocketClient")
            .field("url", &self.url)
...
impl Clone for ArchitectMdWebSocketClient {
    fn clone(&self) -> Self {
        Self {
            url: self.url.clone(),
...
impl ArchitectMdWebSocketClient {
    ...
    pub async fn connect(&mut self) -> ArchitectWsResult<()> {
        ...
        .map_err(|e| ArchitectWsClientError::Transport(e.to_string()))?;
        ...
        let client = loop {
            ...
            if attempt >= MAX_RETRIES {
                return Err(ArchitectWsClientError::Transport(format!(
    ...
    async fn send_cmd(&self, cmd: HandlerCommand) -> ArchitectWsResult<()> {
        let guard = self.cmd_tx.read().await;
        guard
            .send(cmd)
            .map_err(|e| ArchitectWsClientError::ChannelError(e.to_string()))
    }
}

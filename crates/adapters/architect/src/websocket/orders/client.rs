...
//! Orders WebSocket client for Architect.
...
use crate::{
    common::enums::{ArchitectOrderSide, ArchitectTimeInForce},
    websocket::messages::{ArchitectOrdersWsMessage, ArchitectWsPlaceOrder, OrderMetadata},
};

/// Default heartbeat interval in seconds.
const DEFAULT_HEARTBEAT_SECS: u64 = 30;

/// Result type for Architect orders WebSocket operations.
pub type ArchitectOrdersWsResult<T> = Result<T, ArchitectOrdersWsClientError>;

/// Error type for the Architect orders WebSocket client.
#[derive(Debug, Clone)]
pub enum ArchitectOrdersWsClientError {
...
impl core::fmt::Display for ArchitectOrdersWsClientError {
...
impl std::error::Error for ArchitectOrdersWsClientError {}
...
impl Debug for ArchitectOrdersWebSocketClient {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        f.debug_struct("ArchitectOrdersWebSocketClient")
            .field("url", &self.url)
...
impl Clone for ArchitectOrdersWebSocketClient {
    fn clone(&self) -> Self {
        Self {
            url: self.url.clone(),
...
impl ArchitectOrdersWebSocketClient {
    /// Creates a new Architect orders WebSocket client.
    #[must_use]
    ...
    pub async fn connect(&mut self, bearer_token: &str) -> ArchitectOrdersWsResult<()> {
        ...
        .map_err(|e| ArchitectOrdersWsClientError::Transport(e.to_string()))?;
        ...
        let client = loop {
            ...
            if attempt >= MAX_RETRIES {
                return Err(ArchitectOrdersWsClientError::Transport(format!(
        ...
        let (out_tx, out_rx) = tokio::sync::mpsc::unbounded_channel::<ArchitectOrdersWsMessage>();
    ...
    pub async fn place_order(
        ...
        post_only: bool,
        tag: Option<String>,
    ) -> ArchitectOrdersWsResult<i64> {
        let request_id = self.next_request_id();

        let order = ArchitectWsPlaceOrder {
            rid: request_id,
            t: "p".to_string(),
            s: symbol.to_string(),
            d: side,
    ...
    pub async fn cancel_order(&self, order_id: &str) -> ArchitectOrdersWsResult<i64> {
        let request_id = self.next_request_id();
    ...
    pub async fn get_open_orders(&self) -> ArchitectOrdersWsResult<i64> {
        let request_id = self.next_request_id();
    ...
    async fn send_cmd(&self, cmd: HandlerCommand) -> ArchitectOrdersWsResult<()> {
        let guard = self.cmd_tx.read().await;
        guard
            .send(cmd)
            .map_err(|e| ArchitectOrdersWsClientError::ChannelError(e.to_string()))
    }
}
